...
/*
Aqueta API és de KeyAuth
Aquest document ha sigut l'únic que no he fet jo
L'únic que li he modificat és el llenguatge ja que originalment, estava en anglès
Aquest archiu és el que permet iniciar sessió a l'usuari i també protegeix les peticions que fa el programa al servidor, ja que aquestes estàn encriptades i solament el servidor les pot llegir
*/
namespace KeyAuth {
    public class api
    {
        //Et demana el nom del programa, el ID de la conta, el secret del programa i la seva versió per a que així es pugui connectar
        public string name, ownerid, secret, version;
        public api(string name, string ownerid, string secret, string version)
        {
            //Si alguna de les opcions està en blanc, dona error
            ...
            {
                MessageBox.Show("Falten dades per a connectar amb la API");
                Environment.Exit(0);
            }
        ...
        public bool license(string key)
        {
            ...
            if (!json.success)
            {
                MessageBox.Show("Aquesta clau no existeix!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return false;
            }
            else
            {
